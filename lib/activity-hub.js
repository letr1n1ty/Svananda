...
const VALID_KINDS = new Set(["subagent", "workflow", "workflow_agent", "heartbeat", "cron"]);
...
function normalizeEntry(entry, existing) {
  return {
    ...
    // workflow_agent 子节点专属：归属父 wf（parentTaskId）+ 节点标签（label）+
    // phase 弱分组（phaseLabel）+ token 消耗（tokens）。其它 kind 这些字段为 null。
    parentTaskId: pickStr(entry.parentTaskId, existing?.parentTaskId ?? null),
    label: pickStr(entry.label, existing?.label ?? null),
