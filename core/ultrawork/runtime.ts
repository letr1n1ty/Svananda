...
export type UltraworkPacketRunner = (
  input: UltraworkPacketRunnerInput,
) => UltraworkPacketRunnerResult | Promise<UltraworkPacketRunnerResult> | null | undefined;

export type UltraworkTextGenerator = (
  request: UltraworkTextRequest,
) => Promise<UltraworkTextResult | string | null | undefined>;

export type UltraworkArtifactExporter = (
  request: { run: UltraworkRun; artifact: UltraworkArtifact },
) => Promise<UltraworkArtifactExport | null | undefined>;
...
export class OmniUltraworkRuntime {
  ...
  capabilities() {
    return {
      ...
      actions: ["confirm", "continue", "cancel", "run-packet", "run-next-packet", "sync-artifacts"],
  ...
  async runPacket(id: string, packetId: string, input: UltraworkActionInput = {}) {
    ...
    this.record(
      run,
      nextStatus === "failed" ? "work_packet.failed" : "work_packet.completed",
      packet.agent,
      result?.message || `${nextStatus === "failed" ? "Failed" : "Completed"} work packet: ${packet.title}`,
      {
        packetId: packet.id,
        kind: packet.kind,
        actor,
  ...
  async exportArtifacts(id: string, input: UltraworkActionInput = {}) {
    ...
    this.record(run, "artifacts.exported", input.actor || "hana", `Exported ${pending.length} pending artifacts`, {
      artifactIds: pending.map((artifact) => artifact.id),
      reason: input.reason || null,
    });
    this.publishActivity(run);
    this.save();
  ...
  private async addRunnerArtifacts(
    run: UltraworkRun,
    packet: UltraworkWorkPacket,
    runner: string,
    artifacts: UltraworkArtifactDraft[],
  ...
  private async exportArtifact(run: UltraworkRun, artifact: UltraworkArtifact) {
    ...
    try {
      ...
      this.record(run, "artifact.export_failed", "archivist", `Failed to export ${artifact.kind} artifact`, {
        artifactId: artifact.id,
        error: err?.message || String(err),
      });
    }
  ...
  private maybeCompleteRun(run: UltraworkRun) {
    ...
    if (!packets.length || !packets.every((packet) => isTerminalStatus(packet.status))) return;
    ...
    this.record(
      ...
      "reviewer",
      run.status === "failed"
        ? "Failed skeleton Ultrawork run after packet lifecycle advancement"
        : "Completed skeleton Ultrawork run after packet lifecycle advancement",
      {
  ...
  private publishActivity(run: UltraworkRun) {
    const activityHub = this.getActivityHub();
    if (!activityHub || typeof activityHub.upsert !== "function") return;
...
function inferIntent(goal: string): UltraworkIntent {
  ...
  const hits = {
    coding: countSignals(lower, ["repo", "code", "bug", "test", "pr", "typescript", "github", "cli", "server"]),
    product: countSignals(lower, ["prd", "roadmap", "feature", "spec", "product", "ux", "design"]),
    research: countSignals(lower, ["research", "compare", "citation", "source", "survey", "report"]),
    personal_ops: countSignals(lower, ["email", "calendar", "telegram", "memory", "note", "schedule", "todo"]),
  };
